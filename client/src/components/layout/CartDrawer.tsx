  ...
  return (
    ...
    >
      <div
        ref={drawerRef}
        className={cn(
          "absolute bottom-0 left-0 w-full bg-white shadow-[0_-4px_30px_rgba(0,0,0,0.1)] transition-transform duration-300 ease-in-out transform",
          ...
          <>
            ...
            <div className="px-6 md:px-12 pt-10 pb-12 overflow-x-auto cart-scrollbar">
              <div className="flex gap-12 md:gap-16">
                {items.map((item) => (
                  <div key={item.id} className="flex gap-6 shrink-0 min-w-[400px] max-w-[500px]">
                    {/* Product Image */}
                    <div className="group relative w-[200px] h-[240px] md:w-[220px] md:h-[260px] bg-[#F2F2F2] shrink-0 overflow-hidden cursor-pointer">
                      <img
                        src={item.image}
                        alt={item.name}
                        className={cn(
            ...
            {/* Bottom Bar */}
            <div className="px-6 md:px-12 py-8 bg-white border-t border-zinc-100 flex items-center justify-between">
